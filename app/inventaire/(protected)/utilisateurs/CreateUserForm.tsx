...
import { createUserDirect } from "@/lib/inventaire/actions";
...
export function CreateUserForm({ properties }: { properties: { id: string; reference: string; name: string | null }[] }) {
  const [role, setRole] = useState<UserRole>("menage");

  return (
    <ActionForm className="flex flex-wrap items-end gap-2" resetOnSuccess action={createUserDirect}>
      {({ pending, error, success }) => (
        <>
          <div>
            ...
            <input
              name="fullName"
              className="mt-1 w-40 rounded-[10px] border border-black/10 bg-white px-3.5 py-2.5 text-[15px] text-[#1d1d1f] shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition focus:border-[#0071e3] focus:outline-none focus:ring-[3px] focus:ring-[#0071e3]/15"
            />
          </div>
          <div>
            ...
            <input
              ...
              type="email"
              required
              className="mt-1 w-56 rounded-[10px] border border-black/10 bg-white px-3.5 py-2.5 text-[15px] text-[#1d1d1f] shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition focus:border-[#0071e3] focus:outline-none focus:ring-[3px] focus:ring-[#0071e3]/15"
            />
          </div>
          <div>
            <label className="block text-[12px] font-medium text-[#6e6e73]">Mot de passe</label>
            <input
              name="password"
              type="text"
              required
          ...
          {success && <span className="text-sm text-emerald-600">Utilisateur créé.</span>}
