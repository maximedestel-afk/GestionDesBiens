...
const DETAILS_STRING_COLUMNS: [CsvFieldKey, string][] = [
  ...
  ["syndicLotNumber", "syndic_lot_number"],
...
const OWNER_STRING_COLUMNS: [CsvFieldKey, string][] = [
  ...
  ["leaseInitialTerm", "lease_initial_term"],
  ["leaseRenewalTerm", "lease_renewal_term"],
  ["leaseRentFreePeriod", "lease_rent_free_period"],
  ["leaseTenantCompany", "lease_tenant_company"],
  ["leaseTenantDirector", "lease_tenant_director"],
  ["leaseTenantNotes", "lease_tenant_notes"],
  ["leaseSpecialClause1", "lease_special_clause_1"],
  ["leaseSpecialClause2", "lease_special_clause_2"],
  ["leaseTenantTerminationDelay", "lease_tenant_termination_delay"],
  ["leaseTenantNotice", "lease_tenant_notice"],
  ["leaseOwnerTerminationDelay", "lease_owner_termination_delay"],
  ["leaseOwnerNotice", "lease_owner_notice"],
...
const PROPERTY_DETAILS_STRING_FIELDS: [string, string, string][] = [
  ...
  ["syndicLotNumber", "syndic_lot_number", "Numéro de lot"],
...
const PROPERTY_OWNER_STRING_FIELDS: [string, string, string][] = [
  ...
  ["notes", "notes", "Notes"],
  ["leaseStartDate", "lease_start_date", "Date de début du bail"],
  ["leaseInitialTerm", "lease_initial_term", "Durée initiale du bail"],
  ["leaseRenewalTerm", "lease_renewal_term", "Durée tacite reconduction"],
  ["leaseRentFreePeriod", "lease_rent_free_period", "Franchise de loyer"],
  ["leaseTenantCompany", "lease_tenant_company", "Société locataire"],
  ["leaseTenantDirector", "lease_tenant_director", "Directeur locataire"],
  ["leaseTenantNotes", "lease_tenant_notes", "Notes locataire"],
  ["leaseSpecialClause1", "lease_special_clause_1", "Clause Particulière 1"],
  ["leaseSpecialClause2", "lease_special_clause_2", "Clause Particulière 2"],
  ["leaseTenantTerminationDelay", "lease_tenant_termination_delay", "Délai résiliation locataire"],
  ["leaseTenantNotice", "lease_tenant_notice", "Préavis locataire"],
  ["leaseOwnerTerminationDelay", "lease_owner_termination_delay", "Délai résiliation bailleur"],
  ["leaseOwnerNotice", "lease_owner_notice", "Préavis bailleur"],
 ...
 * personnalisé "cleaning_rate" de l'annonce correspondante — retrouvée par
 * référence si aucun ID Guesty n'est renseigné). L'enregistrement côté MGB
 * réussit toujours même si la synchronisation Guesty échoue (source de
 * vérité MGB) : l'erreur est alors stockée sur le bien plutôt que de faire
 * échouer la sauvegarde. */
export async function saveGuestyData(propertyId: string, formData: FormData) {
  const supabase = await createClient();
  await requireUser(supabase);

  const guestyListingIdInput = optionalString(formData.get("guestyListingId"));
  ...
  const { data: existing } = await supabase
    .from("property_data")
    .select("guesty_listing_id, cleaning_rate")
    .eq("property_id", propertyId)
    .maybeSingle();
  ...
  await updatePropertyDetailRow(
    ...
    "property_data",
    propertyId,
    { guesty_listing_id: guestyListingIdInput, cleaning_rate: cleaningRate },
    !!existing
  );

  if (existing?.cleaning_rate !== cleaningRate || existing?.guesty_listing_id !== guestyListingIdInput) {
    await logActivity(supabase, {
      propertyId,
      entityType: "property_data",
      action: "update",
  ...
  if (isGuestyConfigured() && cleaningRate !== null) {
    try {
      let listingId = guestyListingIdInput;
      if (!listingId) {
        const { data: property } = await supabase
          ...
          .eq("id", propertyId)
          .maybeSingle();
        listingId = property ? await findGuestyListingIdByReference(property.reference) : null;
        if (listingId) {
          await supabase.from("property_data").update({ guesty_listing_id: listingId }).eq("property_id", propertyId);
        }
      }
      if (!listingId) throw new Error("Aucune annonce Guesty trouvée pour ce bien (référence sans correspondance).");
...
/** Diagnostic manuel (bouton "Tester la connexion", page DATA) : résout
 * l'annonce Guesty du bien et lit la valeur actuelle de "cleaning_rate" —
 * remonte l'erreur brute de Guesty en cas d'échec, pour ajuster
 * l'intégration au besoin. */
export async function testGuestyConnection(propertyId: string): Promise<string> {
  ...
  let listingId = propertyData?.guesty_listing_id ?? null;
  if (!listingId) {
    ...
    listingId = await findGuestyListingIdByReference(property.reference);
